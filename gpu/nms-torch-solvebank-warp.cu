...
gather_keep_from_mask(bool* __restrict__ keep,
                      const unsigned int* __restrict__ dev_mask,
                      const int n_boxes)
{
    const int col_blocks = ceil_div(n_boxes, threadsPerBlock);
    const int lane       = threadIdx.x;
    extern __shared__ unsigned int removed[];            // size = col_blocks
    for (int i = lane; i < col_blocks; i += warpSize) {
        removed[i] = 0u;
    }
    __syncwarp();                                      // warp barrier
    for (int blk = 0; blk < col_blocks; ++blk) {
        unsigned int removed_val = __shfl_sync(0xffffffff,
                                               removed[blk],   // value written by lane 0
                                               0);              // source lane = 0
        const int i_offset = blk * threadsPerBlock;
        if (i_offset >= n_boxes) break;                // out‑of‑range guard
        #pragma unroll
        for (int lane_it = 0; lane_it < warpSize; ++lane_it) {
            const int idx = i_offset + lane_it;
            if (idx >= n_boxes) break;
            if (!(removed_val & BITMASK_TABLE[lane_it])) {
                if (lane == 0) keep[idx] = true;
                const unsigned int* row_mask = dev_mask + idx * col_blocks;
                for (int j = lane; j < col_blocks; j += warpSize) {
                    removed[j] |= row_mask[j];
                }
                __syncwarp();
                removed_val = __shfl_sync(0xffffffff, removed[blk], 0);
            }
        }
        __syncwarp();
    }
                            ...
                            double iou_threshold)
{
    ...
    dim3 block(32);
    nms_kernel_impl<float><<<grid, block>>>(
